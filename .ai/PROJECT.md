...
根因排名只能回答优先检查谁，尚未解释影响如何经设备关系展开，也不能说明哪些恢复动作仍有必要。局部关系的证据可能不完整且冲突（C1 Local Ambiguity），局部支持组合后也可能不能形成一致事故解释（C2 Global Consistency）。传播关系帮助减少冗余检查或组织恢复动作只是待验证假设，必须以相同反馈、权限和停止规则进行工程验证，不能从图的可视化效果或 PPT 示意计数推出。
...
1. 将事故证据、竞争单设备锚点与条件图解释联系起来，检验锚点不确定性是否影响可恢复结构。
2. 将设备邻接对的局部方向支持与锚点引导的全局图组装分开，输出可追溯解释和证据缺口。
3. 用固定根、Direct Oracle、Shared 和端到端流程区分根定位、关系恢复和图选择的作用；这是评价设计，本身不自动构成独立科学创新。

概念模块：M1 = 事故证据图构建；M2 = 局部传播关系建模；M3 = 锚点引导全局传播图重建。现有代码仍为 Stage 1 PC-STGR 锚点候选和 Stage 2 P0 局部支持/条件 DAG；Stage 1 仅支持 M3 的锚点候选功能，M1 是分散的证据整理职责。历史代码 `propagation/m1`、`m2` 的编号不定义论文模块。方法细节见[项目概览](../docs/project_overview.md)和[最新方案说明](../docs/conferences/WWW2027/2026-09-16_最新方案与文档同步说明.md)。
...
- [项目概览](../docs/project_overview.md)、[最新方案说明](../docs/conferences/WWW2027/2026-09-16_最新方案与文档同步说明.md)：实现与代码组织；二者的优先级列表让位于执行记录。
