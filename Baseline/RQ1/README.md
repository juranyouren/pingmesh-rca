...
**2026-09-21 起出厂配置 `configs/baselines/rq1.json` 改为 `require_coverage: false`。**
原因是当前节点导出一律不带采集覆盖元数据，严格模式下 PCMCI 在 207 例全体上
`input_ineligible`，一行有效结果都产生不出来。这是**计数假设**而非覆盖证明：
它假设“没有导出记录”等于该设备在该时间箱内没有事件，**不代表设备健康或采集完整**。
引用 PCMCI 任何数字时必须同时写明 record-count 口径；需要严格口径时用
`--pcmci-coverage strict`，并接受该口径下大部分案例不适用。
...
**2026-09-21 起出厂预算由 30 类型 / 5000 事件 / 500 设备放宽为 64 / 20000 / 2000**
（`configs/baselines/rq1.json`）。原因是原预算让 207 例中的 11 例仅因规模被判不适用。
放宽的是**计算预算**，不是判定标准：超限仍然记录不适用，不做按标签筛选或静默截断，
也不改 `max_hop` 或上游行为。注意两处口径差异：TTPM 只在**有事件设备**的子图上学习
（`ttpm.py` 的 `self._topo.subgraph(tensor['node'].unique())`），而适配器仍按**全部候选设备**
构造 N×N 稠密拓扑矩阵，因此 `max_devices` 实际拦的是 O(N²) 构图成本而非 EM 计算量——
这个偏差是保守的（宁可不跑也不误跑），但报告 THP 适用率时应说明它计的是候选设备数。
