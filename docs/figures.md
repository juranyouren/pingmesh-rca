...
Pingmesh 拨测发现丢包 → 拉取全链路拓扑（涉及该次故障的全部网络设备，每台带告警/日志/拓扑连接） → 两条并行评分线 → 融合 → 压缩为 prompt → LLM 辅助推理 → 输出根因 IP。

### 各模块详细描述

**模块 A: 数据采集**
- 输入: Pingmesh 丢包告警（source_ip, sink_ip, alarm_time 等）
- 输出: 全链路设备列表，每台含 role, mgmt_ip, alarms[], logs[], linked_from[], linked_to[], cross
...
```
  ...
  │  输出: 全部涉及设备 × {role, mgmt_ip, alarms, logs,          │
...
1. TOP: "Pingmesh Alert" box → "Data Collection" box
   ...
   - Output: "All affected devices: {role, mgmt_ip, alarms[], logs[], linked_from[], linked_to[], cross}"
