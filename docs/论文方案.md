...
> [`PC-STGR设计方案.md`](./PC-STGR设计方案.md)。本文下方的 IC-STGR 结构和
> 159-case 指标用于记录当前已实现、已完成 OOF 的历史状态；在 PC-STGR 完成
> 独立实现和 grouped OOF 前，不得把这些 checkpoint 或指标改名为 PC-STGR 结果。
...
- **Stage 1：IC-STGR（Incident-Conditioned Spatio-Temporal Graph Ranker）根因候选排序**；
- **Stage 2：故障传播路径重建**。

经过方法调研和现有实验比较，本文已经选定 **IC-STGR** 作为 Stage 1 的主方法，后续工作在该模型上优化，不再把 Stage 1 主技术选择作为开放问题。IC-STGR 将设备、事件和 incident 构造成异构图，通过关系感知的多头注意力统一学习空间与时间证据，再输出设备级根因 logit。

IC-STGR 是本项目对所用模型的命名，不是直接复现某一篇同名论文。其技术谱系来自 GAT/HGT 的关系感知图注意力、TGAT 等时序图建模思路以及 RankNet 类排序损失。论文的贡献口径应放在 Pingmesh 场景下的 **Incident 条件化 Device–Event–Incident 异构图、显式时空关系和多正例根因排序适配**，不声称提出了全新的通用注意力机制。
...
flowchart LR
    ...
    subgraph S1["Stage 1：IC-STGR 根因候选排序"]
        direction TB
        G["Device–Event–Incident<br/>时空异构图"]
        E["关系感知多头图注意力"]
...
```text
原始观测
  → Stage 1：构造 Incident 条件化的 Device–Event–Incident 时空异构图
...
## 3. Stage 1：IC-STGR 根因候选排序

### 3.1 技术决策与阶段目标

Stage 1 的主技术已经确定为 **IC-STGR**。给定 Pingmesh 异常端点、物理拓扑和故障窗口内的告警事件，模型输出高召回、带学习分数的根因候选列表：
...
| **IC-STGR** | 5-fold OOF | 159 | 73.58% | **93.71%** | **97.48%** | **选定的 Stage 1 主方法** |
...
主技术不再重新选择。后续实验用于建立基线、验证 IC-STGR 各组成部分的贡献，并在 IC-STGR 上优化：

| 方法/实验 | 简要说明 | 当前状态 | 结果控制 |
| --- | --- | --- | --- |
| **IC-STGR** | Incident 条件化异构图；显式建模 Device、Event、Incident 与时空关系，直接学习设备排序 | 已完成首轮 OOF；主方法 | 73.58 / 93.71 / 97.48 |
...
论文中使用“学习式”或“非透明”描述 IC-STGR。确定性基线用于解释单项证据，LambdaMART 用于回答“统一学习排序是否已经足够”，IC-STGR 则作为完整方法回答“显式事件级时空关系与 incident 条件化是否带来更高的 Top-K 候选覆盖”。
...
- `S_stage1` 是 IC-STGR 输出的设备根因分数，由设备 logit 经 case 内 softmax 得到；
...
| --- | --- | --- |
| 时序排序器 | Stage 1 的历史白盒基线及 IC-STGR 特征参照 | 否 |
| 拓扑排序器 | Stage 1 的历史白盒基线及 IC-STGR 空间关系参照 | 否 |
| 确定性证据融合 | 强基线，不再作为论文 Stage 1 主方法 | 否 |
| LambdaMART | 学习排序强基线，用于检验人工指定特征模式的上限 | 否 |
| IC-STGR | Stage 1 的选定主方法 | 是，Stage 1 核心方法 |
| 多源证据表 | 拆入 IC-STGR 图特征和 Stage 2/M1 episodes | 否 |
| 传播路径重构 | Stage 2/M1 假设传播图重建 | 是 |
| 根因一致性与反证 | Stage 2/M2 联合推断 | 是 |
| 根因排名反向改写 | Stage 2/M2 联合推断 | 是，核心机制 |
| Trust-Tree Gate | 仅作为内部合法性或工程质量检查 | 否 |
| 小模型候选摘要 | 从核心链路移除 | 否 |
| 主 LLM 仲裁 | 可作为扩展对照，不是必要推理环节 | 否 |

因此，论文顶层是两个 Stage，编号模块只出现在 Stage 2。Stage 1 统一称为 IC-STGR，不再把旧的时序/告警—拓扑分支包装成并列论文模块。

## 6. 预期创新点

### 创新点 1：面向 Pingmesh 的 Incident 条件化时空异构图根因排序

将 Device、Event 和 Incident 统一建模，使用显式物理、事件归属、时间先后和全局条件关系进行关系感知消息传递，并以多正例排序目标形成高召回的初始根因候选。创新点是场景建模与排序任务适配，而不是宣称提出新的通用注意力算子。
...
4. Stage 1：IC-STGR 异构图构造、关系感知编码与多正例排序；
...
| 新架构 | 当前实现 |
| --- | --- |
| Stage 1 异构图构造 | `Sys/RootCauseAnalyze/stage1/neural_graph.py` |
| Stage 1 关系感知编码、排序头与损失 | `Sys/RootCauseAnalyze/stage1/neural_model.py` |
| Stage 1 OOF 训练与无标签推理 | `Sys/RootCauseAnalyze/stage1/neural_pipeline.py` |
| Stage 1 论文实验入口 | `scripts/run_paper_05_spatiotemporal_graph.sh` |
...
当前实现已按接口边界完成拆分：IC-STGR 输出 OOF 或无标签推理的 `initial_root_rankings`；M1 独立输出一张根因无关的 `hypothesis_graph`；M2 输出每个根因的 `explanation_score`、对应传播图和 `final_root_rankings`。最终排序只采用 Stage 1 得分与解释性得分两个加权分量。

## 9. 研究边界

- 推理过程不得读取根因或路径真值；
- 论文主表中的 IC-STGR 分数必须来自 OOF 预测，不能使用全量标签训练 checkpoint 对同一批 case 的回代结果；
...
- 数据预处理、Gate、缓存、拒答和人工复核不包装成额外论文 Stage 或 Module。
