...
为检验候选传播图是否含有独立于 Stage-1 分数的根因判别信息，仓库提供两项
可开关优化，但在 grouped-OOF 结果确认前不替换上述 P0 主方法：

1. 将根无关的 `A→B / B→A / No Direct` 三状态概率追加到 PC-STGR 物理边；
2. 为 Stage-1 Top-K 每个候选根恢复 P0 DAG，抽取覆盖、路径、边支持、
   No-Direct margin、证据绑定、矛盾与复杂度统计，使用小型 residual MLP 产生
   logit 修正，并以 incident 内 listwise softmax 训练。

消融必须包含只读取 Stage-1 分数的 MLP 控制组。传播图 MLP 相对该控制组的
增量，才可归因于传播图信息。外折验证使用 Stage-1 OOF 预测，重排器训练使用
对应 Stage-1 外折训练模型的训练侧预测；传播图标签不参与该实验。

### 5.8 候选条件传播图 K 次验证

候选条件验证实验不再把传播图压缩为人工统计量。首先由带根无关三状态边概率
的 PC-STGR 生成 Top-K；随后对每个候选根分别恢复 P0 传播 DAG，将选中方向
编码为 `[1,0,0]`、反向观察编码为 `[0,1,0]`，未选中边使用全零 mask。
验证器对 K 张硬边图分别执行一次 PC-STGR 前向，并计算：

```text
q(r) = logit_r(r | G_r) - max_{v != r} logit_r(v | G_r)
S_final(r) = S_stage1(r) + tanh(gate) * lambda_max * q(r)
```

`argmax_v logit_r(v|G_r)=r` 作为候选自洽性的可审计判据，连续 margin 用于
最终排序。训练在 incident 内对 K 个 `S_final` 使用 listwise 交叉熵，并增加
候选 margin 的辅助 listwise 损失。外折验证严格加载相同 Stage-1 fold 的模型、
词表和训练侧预测；推理阶段不读取根标签或传播标签。该方案需分别报告冻结
Stage-1 骨干与全量微调结果，以区分零样本结构检验和有监督候选验证的收益。
