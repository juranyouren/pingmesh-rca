# PC-STGR 根因候选排序实现记录（非当前论文方案）

> **状态更新（2026-08-27）：本文档已退出论文主方案。**
>
> 当前论文唯一主要任务是异构故障传播图重构，以
> [`论文方案.md`](./论文方案.md) 为准。目标方法在同一异构图上联合识别根因与
> 传播图，不要求 PC-STGR 作为前置输入。PC-STGR 仅保留为串联系统对比、可选
> 根因候选生成器或附录基线；不得再把它描述为论文主方法、第一阶段核心贡献，
> 或据此恢复旧的“根因排序 + 传播图 + 根因重排”两阶段论文结构。
>
> 以下内容仅用于维护已经存在的 PC-STGR 代码和复现实验契约。状态：代码迁移已完成，
> 独立 grouped OOF 实验待完成。原决策日期：2026-08-18。

## 1. 文档目的

本文档记录 PC-STGR 的方法定义、数据假设、特征工程、图结构、网络结构、向量维度、训练目标和实验约束，仅作为候选排序实现的技术记录。后续传播图论文的定位、贡献、方法结构和主实验不得以本文档为依据。
...
PC-STGR 是本项目中的候选排序器名称。若在扩展实验或附录中介绍它，只能描述为 Pingmesh 路径条件化 Device–Event 排序实现，不得列为传播图论文的创新点，也不声称提出新的通用注意力机制。
