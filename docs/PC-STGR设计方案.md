...
PC-STGR 是本项目方法名。论文贡献口径应放在 Pingmesh 场景下的路径条件化 Device–Event 图、显式时空关系和 case 内设备排序适配，不声称提出新的通用注意力机制。
...
Stage 1 中 `combined_score` 与 `neural_score` 都是 PC-STGR 的 case 内概率。该输出接口应继续与后续 Stage 2 兼容。
