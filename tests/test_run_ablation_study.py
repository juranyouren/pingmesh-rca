...
def test_m123_view_keeps_semantic_summary_and_drops_prompt_noise():
    row = _table()["rows"][0]
    row["summary_context"] = {
        "prompt_tokens": 100,
        "parse_mode": "json",
...
def test_all_llm_modes_force_every_gate_route_and_use_distinct_prompts(tmp_path):
    ...
    assert '"initial_ranking"' not in rerank["prompt"]
    assert "Gate 上下文" not in rerank["prompt"]
    assert "故障概况" not in rerank["prompt"]
    assert "实验模式" not in rerank["prompt"]
    assert '"semantic_summary"' in rerank["prompt"]
    assert '"summary_context"' not in rerank["prompt"]
    assert '"log_count"' not in rerank["prompt"]
    assert '"raw_temporal_score"' in rerank["prompt"]
    assert '"burst_score"' not in rerank["prompt"]
