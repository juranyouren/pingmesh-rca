...
**重要**：此前 87.41% / 76.22% 因 temporal_score 从 label.json 泄漏标注数据而虚高。修正后结果：

| 组合 | Top-1 | Top-3 | Top-5 |
|------|-------|-------|-------|
| `[1,2]` topo+temporal (llm权重) | **55.94%** | 60.14% | 64.34% |
| `[1,2]` topo+temporal (manual权重) | 54.55% | 58.04% | 64.34% |
| `[2]` temporal only | 49.65% | 58.04% | 58.74% |
| `[1]` topo only (manual权重) | 42.66% | 54.55% | 60.14% |
| `[1]` topo only (llm权重) | 39.16% | 51.75% | 60.84% |

核心结论：topo+temporal 互补（+6pp over temporal alone），两个信号都有效且非泄漏。
