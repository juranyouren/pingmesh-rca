...
from Sys_v1.RootCauseAnalyze.prompt_templates import EVIDENCE_RCA_PROMPT
from Sys_v1.RootCauseAnalyze.skills.provider import BuiltinSkillProvider
from Sys_v1.utils.case_utils import find_full_link_file
from Sys_v1.utils.io_utils import load_json, save_json
from Sys_v1.utils.npu_utils import wait_npu_memory, get_npu_memory_info, list_npu_processes
...
def _case_cache_key(
    ...
    *,
    ...
    from Sys_v1.RootCauseAnalyze.gate.evidence import EVIDENCE_ORGANIZATION_VERSION
    from Sys_v1.RootCauseAnalyze.gate.node_summarizer import SUMMARY_PROMPT_VERSION

    if top_k is None:
        top_k = config.temporal.top_k
    material = (
        f"{EVIDENCE_ORGANIZATION_VERSION}|{SUMMARY_PROMPT_VERSION}|"
        f"ablation={ablation}|top_k={top_k}|neighbor_mode={neighbor_alarm_mode}|"
        f"neighbor_devices={max_neighbor_devices}|neighbor_alarms={max_neighbor_alarms}|"
...
def _format_skilled_prompt(
    ...
    prompt = EVIDENCE_RCA_PROMPT.format(
        SKILLRET=skill_ret,
        INFO=info,
        NODES=nodes,
...
class SkilledAnalyzer:
    def __init__(self, model_path=None, ASCEND_RT_VISIBLE_DEVICES=None, skill_json_path=None, short=None, top_k=None,
                 ...
                 summary_max_tokens=1024, summary_cache_dir=None, ablation="m123",
                 neighbor_alarm_mode=None, max_neighbor_devices=None,
                 max_neighbor_alarms=None, summary_context_max_chars=None):
        """
        ...
        self.ablation_spec = get_ablation_spec(ablation)
        self.ablation = self.ablation_spec.name
        ...
        self.short=short
        self.top_k = top_k
        # The ablation definition owns routing semantics. The legacy flag is
        # accepted for CLI compatibility but cannot make M1 run M3 or disable
        # M3 in the three variants that include it.
        self.confidence_gate_enabled = bool(self.ablation_spec.enable_gate)
        ...
        self.summarize_nodes_enabled = bool(summarize_nodes and self.ablation_spec.enable_m2)
        ...
        self.neighbor_alarm_mode = neighbor_alarm_mode or config.evidence.neighbor_alarm_mode
        self.max_neighbor_devices = int(
            config.evidence.max_neighbor_devices
            if max_neighbor_devices is None
            else max_neighbor_devices
        )
        self.max_neighbor_alarms = int(
            config.evidence.max_neighbor_alarms
            if max_neighbor_alarms is None
            else max_neighbor_alarms
        )
        self.summary_context_max_chars = int(
            config.evidence.summary_context_max_chars
            if summary_context_max_chars is None
            else summary_context_max_chars
        )
        self.llm = None
    ...
    def _summarize_candidate_detail(self, candidate_detail: str) -> str:
        ...
        return summarize_nodes_with(
            ...
            max_prompt_chars=self.summary_context_max_chars,
        )
    ...
    def _load_cached_node_summary(self, dirpath: str, fallback_detail: str) -> str:
        ...
        key = _case_cache_key(
            ...
            neighbor_alarm_mode=self.neighbor_alarm_mode,
            max_neighbor_devices=self.max_neighbor_devices,
            max_neighbor_alarms=self.max_neighbor_alarms,
        )
        ...
        try:
            data = load_json(path)
            from Sys_v1.RootCauseAnalyze.gate.node_summarizer import strip_reasoning_content
    ...
    def _build_final_prompt(self, original_prompt: str, selected_skill_ids: list, dirpath: str):
        ...
        del original_prompt, selected_skill_ids
        ...
        skill_ret, info_data, detail_compact, detail_raw, skill_ips = build_fused_evidence(
            ...
            top_k=self.top_k,
            skill_ids=self.ablation_spec.skill_ids,
            candidate_strategy=self.ablation_spec.candidate_strategy,
            enable_m2=self.ablation_spec.enable_m2,
            neighbor_alarm_mode=self.neighbor_alarm_mode,
            max_neighbor_devices=self.max_neighbor_devices,
            max_neighbor_alarms=self.max_neighbor_alarms,
        )

        if not self.ablation_spec.enable_m3:
            gate = {
                "enabled": False,
                "decision": "bypass_llm",
                "route": "topo",
                "reason": "m1_direct_topology_ranking",
                "recommended_ips": skill_ips[:3],
            }
        elif self.confidence_gate_enabled:
            gate = assess_gate(
                skill_ret,
        ...
        gate["ablation"] = self.ablation
        gate["enabled_modules"] = [
            f"M{module_id}" for module_id in self.ablation_spec.skill_ids
        ] + (["M3"] if self.ablation_spec.enable_m3 else [])

        if gate.get("decision") in ("bypass_llm", "operator_review"):
            # Semantic compression and both LLMs are unnecessary on this route.
            gate_tag = (
                "CONFIDENCE_GATE_BYPASS"
                if gate.get("decision") == "bypass_llm"
                else "CONFIDENCE_GATE_OPERATOR_REVIEW"
            )
            final_prompt = (
                f"{gate_tag}\n"
                "# 故障概况\n"
        ...
        if self.ablation_spec.enable_m2 and self.summary_cache_dir:
            detail_for_llm = self._load_cached_node_summary(dirpath, detail_compact)
            detail_is_summary = detail_for_llm != detail_compact
        elif self.ablation_spec.enable_m2 and self.summarize_nodes_enabled:
    ...
    def batch_infer(
        ...
        try:
            ...
            for dirpath, original_p in zip(dirpaths, prompts):
                skill_ids_list.append([str(skill_id) for skill_id in self.ablation_spec.skill_ids])
                ...
                # Labels are evaluation-only and are never read during inference.
                gt_ips_list.append([])
                if gate.get("decision") in ("bypass_llm", "operator_review"):
...
def generate_prompts(root_path: str) -> tuple:
    ...
    for dirpath, dirnames, filenames in os.walk(root_path):
        ...
        if info_file and full_link_file:
            ...
            try:
                # Validate inputs here; the module-specific prompt is built
                # later from bounded evidence rather than the full raw graph.
                load_json(node_path)
                load_json(info_path)
...
def worker_process(worker_id: int, npus: str, dirpaths_chunk: list, prompts_chunk: list,
                   target_skill_ids: list, batch_size: int = 8, short=0, top_k=10,
                   confidence_gate=False, confidence_high_margin=15.0,
                   confidence_agreement_margin=8.0, summarize_nodes=False,
                   summary_model_path=None, summary_npu_cards=None,
                   summary_max_tokens=1024, summary_cache_dir=None,
                   print_first_prompt=False, ablation="m123",
                   neighbor_alarm_mode=None, max_neighbor_devices=None,
                   max_neighbor_alarms=None, summary_context_max_chars=None) -> dict:
    import os
    ...
    analyzer = SkilledAnalyzer(
        ...
        ablation=ablation,
        neighbor_alarm_mode=neighbor_alarm_mode,
        max_neighbor_devices=max_neighbor_devices,
        max_neighbor_alarms=max_neighbor_alarms,
        summary_context_max_chars=summary_context_max_chars,
    )
...
def distribute_inference_tasks(dirpath_list: list, prompt_list: list, npu_list: list,
                               target_skill_ids: list, batch_size: int = 8, short=0,
                               top_k=10, confidence_gate=False,
                               confidence_high_margin=15.0,
                               confidence_agreement_margin=8.0,
                               summarize_nodes=False, summary_model_path=None,
                               summary_npu_cards=None, summary_max_tokens=1024,
                               summary_cache_dir=None, print_first_prompt=False,
                               ablation="m123", neighbor_alarm_mode=None,
                               max_neighbor_devices=None, max_neighbor_alarms=None,
                               summary_context_max_chars=None) -> dict:
    total_tasks = len(prompt_list)
    if total_tasks == 0:
        return {}

    if ablation == "m1":
        # M1 is deterministic and never initializes either model.
        num_instances = 1
        npu_groups = [str(npu_list[0]) if npu_list else ""]
    else:
        num_instances = len(npu_list) // 2
        if num_instances == 0:
            raise ValueError("At least two NPU cards are required for each LLM inference worker.")
...
if __name__ == "__main__":
    ...
    p.add_argument("--ablation", choices=["m1", "m1_m3", "m2_m3", "m123"],
                   default="m123",
                   help="运行模块组合：m1、m1_m3、m2_m3 或完整 m123")
    ...
    p.add_argument("--neighbor-alarm-mode",
                   choices=["none", "highest_weight", "all"],
                   default=config.evidence.neighbor_alarm_mode,
                   help="每台目标设备纳入的邻接告警策略")
    p.add_argument("--max-neighbor-devices", type=int,
                   default=config.evidence.max_neighbor_devices,
                   help="单设备 prompt 最多保留的有告警邻接设备数")
    p.add_argument("--max-neighbor-alarms", type=int,
                   default=config.evidence.max_neighbor_alarms,
                   help="neighbor-alarm-mode=all 时每个邻居最多保留的告警数")
    p.add_argument("--summary-context-max-chars", type=int,
                   default=config.evidence.summary_context_max_chars,
                   help="小模型单设备 prompt 的字符上限")
    ...
    ablation_spec = get_ablation_spec(args.ablation)
    target_skill_ids = [str(sid) for sid in ablation_spec.skill_ids]
    ...
    if prompts:
        ...
        final_results = distribute_inference_tasks(
            ...
            neighbor_alarm_mode=args.neighbor_alarm_mode,
            max_neighbor_devices=args.max_neighbor_devices,
            max_neighbor_alarms=args.max_neighbor_alarms,
            summary_context_max_chars=args.summary_context_max_chars,
        )
