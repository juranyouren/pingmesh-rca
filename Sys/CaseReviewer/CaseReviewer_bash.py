...
# 导入您设计好的两类Prompt
from utils.prompts import CASE_REVIEW_SINGLE, CASE_REVIEW_ALL,SKILL_GEN,BAD_CASE_REVIEW,CATAGORIZE,SKILL_GEN_HISTORY
...
def skill_gen_pmt(sk_gd):
    ...
    for mist,gd in sk_gd.items():
        ...
        pmts.append(pmt)
    return pmts
def skill_gen_his_pmt(sk_gd):
    # pmts=[]
    # se =SkillExecutor()
    # cur_sks=se.get_llm_skill_prompt()
    # for mist,gd in sk_gd.items():
    #     pmt=SKILL_GEN_HISTORY.format(
    #         current_skills=cur_sks,
    #         logic_category=mist,
    #         logic_rules=gd
    #     )
    #     pmts.append(pmt)
    ...
    se =SkillExecutor()
    cur_sks=se.get_llm_skill_prompt()
    for cata in sk_gd.get("categories"):
        pmt=SKILL_GEN_HISTORY.format(
            current_skills=cur_sks,
            logic_category=cata.get("description"),
            logic_rules=cata.get("suggested_skills")
        )
        pmts.append(pmt)
    return pmts
        ...
        for jb in json_blocks:
            ...
            for key,ls in result.items():
                if key in d:
                    ...
                    d.pop(key)
            result.update(d)
    save_json(result,f"{os.path.dirname(path)}/sk_guide.json")
    return result
...
if __name__ == "__main__":
    ...
    parser.add_argument("--failure_cases_path", type=str, required=True, help="ranking_failures.json 的路径")
    args = parser.parse_args()
    MODEL_PATH="/usr/share/large_language_models/DeepSeek-R1-Distill-Qwen-32B"
    available_npus = [0,1,2, 3, 4, 5, 6, 7]
    # 替换原有硬编码
    root_path = args.failure_cases_path
    ...
    review_texts=load_json(os.path.join(save_dir, "exps.json"))
    ...
    extract_skills_gd(skill_save_path)
    # # ================= 阶段三：Skill生成 =================
    print(f"\n====== [阶段三] skill生成======")
    start_time = time.time()
    sk_gd=load_json(os.path.join(save_dir, "sk_guide.json"))
    # 2. 按 Token 限制分块 (单块限制在 8500 tokens 左右，确保总计不超过 10000)
    prompts=skill_gen_his_pmt(sk_gd)
    save_json(prompts,os.path.join(save_dir, "pmt_gensk.json"))
    # 3. 开启独立进程进行推理
    ctx = mp.get_context('spawn')
    with ProcessPoolExecutor(max_workers=1, mp_context=ctx) as executor:
        future = executor.submit(
