...
def build_fused_evidence(node_list, info, dirpath,
    ...
    Returns:
        (evidence_str, info_brief, candidate_detail) 三段紧凑文本。
    """
    if not node_list:
    ...
    return evidence_str, info_brief, candidate_detail
...
def _build_candidate_detail(candidate_ips, node_by_ip):
    ...
    for ip in candidate_ips:
        ...
        blocks.append(f"[{ip}] (role={node.get('role', 'UNKNOWN')}): {detail}")
