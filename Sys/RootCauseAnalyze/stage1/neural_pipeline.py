...
def _inner_graph_split(
    cases: Sequence[RawCase], graphs: Sequence[Any], *, seed: int
) -> tuple[List[Any], List[Any]]:
    """Select epochs inside the outer training partition only."""
    ...
    try:
        ...
        return list(graphs), []
    training_graphs = [graph for index, graph in enumerate(graphs) if index not in validation]
    validation_graphs = [graph for index, graph in enumerate(graphs) if index in validation]
    if not training_graphs or not validation_graphs:
        return list(graphs), []
...
def run_cross_validation(args: argparse.Namespace) -> str:
    ...
    for fold_number, validation_indices in enumerate(folds, 1):
        ...
        inner_training_graphs, inner_validation_graphs = _inner_graph_split(
            training_cases,
            training_graphs,
            seed=args.seed + fold_number,
        )
        if inner_validation_graphs:
            (
                _selection_model,
                best_epoch,
                selection_history,
            ) = backend.train_model(
                inner_training_graphs,
                inner_validation_graphs,
        ...
        model, _outer_epoch, outer_training_history = backend.train_model(
            training_graphs,
            [],
            vocabulary_size=len(vocabulary.itos),
            model_config=model_config,
        ...
        histories[str(fold_number)] = {
            ...
            "outer_training": outer_training_history,
        }
