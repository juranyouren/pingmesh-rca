...
echo ""
echo "--- TraceRCA ---"
python Baseline/TraceRCA/TraceRCAnalyzer.py "${PINGMESH_DATA}" | tail -5
tracerca_res="${PINGMESH_RESULTS}/$(ls -t ${PINGMESH_RESULTS}/ | grep tracerca_baseline | head -1)/res.json"
...
echo ""
echo "--- NetEventCause ---"
python Baseline/NetEventCause/NECAnalyzer.py "${PINGMESH_DATA}" | tail -5
nec_res="${PINGMESH_RESULTS}/$(ls -t ${PINGMESH_RESULTS}/ | grep neceventcause_baseline | head -1)/res.json"
...
echo ""
echo "--- BiAn ---"
python Baseline/BiAn/BiAnalyzer.py "${PINGMESH_DATA}" "${PINGMESH_NPU_CARDS}" | tail -5
bian_res="${PINGMESH_RESULTS}/$(ls -t ${PINGMESH_RESULTS}/ | grep bian_baseline | head -1)/res.json"
